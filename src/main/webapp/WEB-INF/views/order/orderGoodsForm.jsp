...
#submit {
	background-image : url("${contextPath}/resources/image/btn_gulje.jpg");
	width : 90px;
	height: 30px;
...
function fn_process_pay_order(){
    ...
    var i_zipcode=document.createElement("input");
    var i_roadAddress=document.createElement("input");
    var i_jibunAddress=document.createElement("input");
    var i_namujiAddress=document.createElement("input");
    ...
    var t_delivery_method=document.getElementById("delivery_method");
  
	var t_receiver_name=document.getElementById("receiver_name");	
	var t_hp1=document.getElementById("hp1");
	var t_hp2=document.getElementById("hp2");
	var t_hp3=document.getElementById("hp3");
	var t_zipcode=document.getElementById("zipcode");
	var t_roadAddress=document.getElementById("roadAddress");
	var t_jibunAddress=document.getElementById("jibunAddress");
	var t_namujiAddress=document.getElementById("namujiAddress");
	var t_delivery_message=document.getElementById("delivery_message");
	var t_delivery_method=document.getElementById("delivery_method");
	var t_gift_wrapping=document.getElementById("gift_wrapping");
	var t_pay_method = document.getElementById("pay_method");
	var t_cart_com_name=document.getElementById("card_com_name");
	var t_card_pay_month=document.getElementById("card_pay_month");
	
	i_delivery_method = t_delivery_method;
	i_receiver_name = t_receiver_name;
	i_receiver_hp1 = t_hp1;
	i_receiver_hp2 = t_hp2;
	i_receiver_hp3 = t_hp3;
	i_zipcode = t_zipcode;
	i_roadAddress = t_roadAddress;
	i_jibunAddress = t_jibunAddress;
	i_namujiAddress = t_namujiAddress;
	i_delivery_message = t_delivery_message;
	i_delivery_method = t_delivery_method;
	i_gift_wrapping = t_gift_wrapping;
	i_pay_method = t_pay_method;
	i_cart_com_name = t_cart_com_name;
	i_cart_pay_month = t_card_pay_month;	
	...
	formObj.appendChild(i_roadAddress);
	formObj.appendChild(i_jibunAddress);
	formObj.appendChild(i_namujiAddress);
	formObj.appendChild(i_delivery_message);
	formObj.appendChild(i_delivery_method);
	formObj.appendChild(i_gift_wrapping);
	formObj.appendChild(i_pay_method);
	formObj.appendChild(i_cart_com_name);
	formObj.appendChild(i_cart_pay_month);
		
	alert(i_delivery_method.value);
...
<form  name="form_order" action="${contextPath}/order/payToOrderGoods.do" method="post">	
				...
				<c:forEach var="item" items="${myOrderList }">
					<td class="goods_image">
					  <a href="${contextPath}/goods/goodsDetail.do?goods_id=${item.goods_id }">
					    <img width="75" alt=""  src="${contextPath}/thumbnails.do?goods_id=${item.goods_id}&goods_fileName=${item.goods_fileName}">
					    <input   type="hidden" id="h_goods_id" name="goods_id" value="${item.goods_id }" />
					    <input   type="hidden" id="h_goods_fileName" name="goods_fileName" value="${item.goods_fileName }" />
					  </a>
					</td>
					<td>
					  <h2>
					     <a href="${pageContext.request.contextPath}/goods/goods.do?command=goods_detail&goods_id=${item.goods_id }">${item.goods_title }</A>
					      <input   type="hidden" id="h_goods_title" name="goods_title" value="${item.goods_title }" />
					  </h2>
					</td>
					<td>
					  <h2>${item.cart_goods_qty }개<h2>
					    <input   type="hidden" id="h_order_goods_qty" name="order_goods_qty" value="${item.cart_goods_qty}" />
					</td>
					<td><h2>${item.goods_sales_price}원 (10% 할인)</h2></td>
					<td><h2>0원</h2></td>
					<td><h2>${1500 *item.cart_goods_qty}원</h2></td>
					<td>
					  <h2>${item.goods_sales_price * item.cart_goods_qty}원</h2>
					  <input  type="hidden" id="h_each_goods_price"  name="each_goods_price" value="${item.goods_sales_price * item.cart_goods_qty}" />
					</td>
			</tr>
			<c:set var="final_total_order_price"
				value="${final_total_order_price+ item.goods_sales_price* item.cart_goods_qty}" />
			<c:set var="total_order_price"
				value="${total_order_price+ item.goods_sales_price* item.cart_goods_qty}" />
			<c:set var="total_order_goods_qty"
				value="${total_order_goods_qty+item.cart_goods_qty }" />
			</c:forEach>
		</tbody>
	</table>
	<div class="clear"></div>
		...
		<table>
			<tbody>
				...
				<tr class="dot_line">
					...
					<td><select id="hp1" name="receiver_hp1">
							<option>없음</option>
					 ...
					 - <input size="10px" type="text" id="hp2" name="receiver_hp2" value="${orderer.member_hp2 }"> 
					 - <input size="10px" type="text" id="hp3" name="receiver_hp3" value="${orderer.member_hp3 }"><br><br> 
					  <input type="hidden" id="h_hp1" name="h_member_hp1" value="${orderer.member_hp1 }" /> 
					  <input type="hidden" id="h_hp2" name="h_member_hp2" value="${orderer.member_hp2 }" /> 
					  <input type="hidden" id="h_hp3" name="h_member_hp3"  value="${orderer.member_hp3 }" />
					  <c:set  var="orderer_hp" value="${orderer.member_hp1}-${orderer.member_hp2}-${orderer.member_hp3 }"/>

				  </tr>
				  
				<tr class="dot_line">
					<td class="fixed_join">유선전화(선택)</td>
					<td><select id="tel1" name="member_tel1">
					...
					</select> - <input size="10px" type="text" id="tel2" name="member_tel2" value="${orderer.member_tel2 }"> - <input size="10px" type="text" id="tel3" name="member_tel3" value="${orderer.member_tel3 }">
					</td>
					<input type="hidden" id="h_tel1" name="h_member_tel1" value="${orderer.member_tel1 }" />
					<input type="hidden" id="h_tel2" name="h_member_tel2"	value="${orderer.member_tel2 }" />
					<input type="hidden" id="h_tel3" name="h_member_tel3"value="${orderer.member_tel3 }" />
				</tr>
				
				<tr class="dot_line">
					<td class="fixed_join">주소</td>
					<td><input type="text" id="zipcode" name="post_code" size="5"
						value="${orderer.post_code }"> 
						<a href="javascript:execDaumPostcode()">우편번호검색</a> <br>
						<p>
							지번 주소:<br>
							<input type="text" id="roadAddress" name="parcel_address" size="50" value="${orderer.parcel_address }" /><br>
							<br> 도로명 주소: 
							   <input type="text" id="jibunAddress" name="street_address" size="50"
								              value="${orderer.street_address }" /><br>
							<br> 나머지 주소: 
							   <input type="text" id="namujiAddress"  name="detail_address" size="50"
								     value="${orderer.detail_address }" /> 
						</p> 
						 <input type="hidden" id="h_zipcode" name="h_post_code" value="${orderer.post_code }" /> 
						 <input type="hidden"  id="h_roadAddress" name="h_parcel_address"  value="${orderer.parcel_address }" /> 
						 <input type="hidden"  id="h_jibunAddress" name="h_street_address" value="${orderer.street_address }" /> 
						 <input type="hidden"  id="h_namujiAddress" name="h_detail_address" value="${orderer.detail_address }" />
					</td>
				</tr>
				<tr class="dot_line">
				...
				<tr class="dot_line">
					...
					<td>
						...
						<input type="radio" id="gift_wrapping" name="gift_wrapping" value="no" checked>아니요
					</td>
				</tboby>
			</table>
	</div>
		   ...
		   <tbody>
			 <tr class="dot_line">
			  ...
			  <tr class="dot_line">
				<td ><h2>핸드폰</h2></td>
				<td>
				 <input type="text" value="${orderer.member_hp1}-${orderer.member_hp2}-${orderer.member_hp3}" size="15" />
				 <input type="hidden"  id="orderer_hp1" name="orderer_hp1" value="${orderer.member_hp1}" />
				 <input type="hidden"  id="orderer_hp2" name="orderer_hp2" value="${orderer.member_hp2}" />
				 <input type="hidden"  id="orderer_hp3" name="orderer_hp3" value="${orderer.member_hp3}" />
				</td>
			  </tr>
			  <tr class="dot_line">
				<td ><h2>이메일</h2></td>
				<td>
				 ...
				 <input type="hidden"  id="orderer_email1" name="orderer_email1" value="${orderer.member_email1}" />
				 <input type="hidden"  id="orderer_email2" name="orderer_email2" value="${orderer.member_email2}" />
				</td>
			  </tr>
		   </tbody>
		</table>
			...
			<tr cellpadding=40 align=center>
				...
				<td>
					<p id="p_final_totalPrice">
						<font size="3">${final_total_order_price }원 </font>
   ...
   <div class="clear"></div>
	...
	<center>
		<br>
		<br> <input type="submit" id="submit" value=""/><!--  <a href="javascript:fn_show_order_detail();"> -->
